...
//Julio 30 2012
//Funciones JavaScript para el modulo de logistica
...
$(function(){
	...
	$("#iddepto").cargarCombo("idmpio","tematica/actualizarmunicipios");
	$("#iddeptoest").cargarCombo("idmpioest","tematica/actualizarmunicipios");
	...
	function obtenerSalario(){
		...
		$.ajax({
			type: "POST",
			url: base_url + "tematica/obtenerSalario",
			dataType: "json",
			cache: false,
	...
	if((typeof(numord)!="undefined")&&(typeof(numest)!="undefined")){
		...
		$.ajax({
			type: "POST",
			url: base_url + "tematica/obtenerObservaciones/"+numord+"/"+numest,
	...
	//Busca una fuente dentro del directorio de fuentes (formularios).	
	$("#btnBuscarLOG").click(function(){
		var opcion = $("input[name='radBusqueda']:checked").val();
		var buscar = $("#txtBuscar").val();
		$.ajax({
			type: "POST",
			url: base_url + "tematica/buscarFuentes",
			data: {'opcion': opcion,
				   'buscar': buscar
		    },
			dataType: "html", 
	...
	$("#cmbFuenteLOG").change(function(){
		$.ajax({
			type: "POST",
			url: base_url + "tematica/obtenerDatosFuente",
	...
	$("#btnAgregarLOG").click(function(){
		$("#agregarFuenteLOG").dialog({
			width: 680,
	...
	//Abre el dialogo para eliminar el registro de una fuente.
	$("#btnEliminarLOG").click(function(){
		$("#removerFuenteLOG").dialog({
			width: 680,
	...
	//Remover fuente (Elimina una fuentes del directorio de fuentes)
	$("#btnRemoverFTELOG").click(function(){
		$.ajax({
			type: "POST",
			url: base_url + "tematica/removerFuente",
			data: $("#frmRemoverFTE").serialize(),
			dataType: "html", 
		    cache: false,
			success: function(data){
	...
	//Valida el envío del formulario de agregar fuente
	//Valida el envío del formulario de agregar fuentes
	$("#btnAgregarFuenteLOG").click(function(){
		$("#frmAgregarFTE").validate({
			...
			submitHandler: function(form) {
				$.ajax({
					type: "POST",
					url: base_url + "logistica/insertarFuente",
				...
				$("#agregarFuenteLOG").dialog('close');
			}
		});
	});
	
	//Abrir la ventana para obtener las observaciones de logística para el capitulo I (Caratula)
	$("#btnOBSLogisticaI").click(function(){		
		if ($("#frmModuloI").valid()){
			$("#hddCapitulo").val(1);
			$("#txaobservacionesTEM").val("");
			$("#observacionesTEM").dialog({
				width: 480,
				title: 'Observaciones de Logistica',
				modal: true			
			});
	...
	//Abrir la ventana para obtener las observaciones de logística para el capitulo II (Movimiento mensual)
	$("#btnOBSLogisticaII").click(function(){		
	...
	//Abrir la ventana para obtener las observaciones de logística para el capitulo III (Ingresos Netos Operacionales)
	$("#btnOBSLogisticaIII").click(function(){		
	...
	//Abrir la ventana para obtener las observaciones de logística para el capitulo IV (Personal ocupado promedio)
	$("#btnOBSLogisticaIV").click(function(){		
	...
	$("#btnOBSLogisticaV").click(function(){
		if ($("#frmModuloV").valid()){
			$("#hddCapitulo").val(5);
			$("#txaobservacionesTEM").val("");
			$("#observacionesTEM").dialog({
	...
	//Abrir la ventana para obtener las observaciones de logística para el envío del formulario (Envío formulario)
	$("#btnOBSEnvioLOG").click(function(){		
	...
	//Ejecutar las funciones AJAX para guardar los comentarios de logistica y los datos modificados del formulario
	$("#btnGuardarCriticaLOG").click(function(){
		...
		else
		{
			$.ajax({
				type: "POST",
				url: base_url + "tematica/guardarCapitulo",
				data: {'observacion': $("#txaobservacionesTEM").val(), 'capitulo':$("#hddCapitulo").val(), 'form': $(formulario).serialize()},
				dataType: "html",
			    cache: false,
				success: function(data){
					$("#observacionesTEM").dialog("close");
