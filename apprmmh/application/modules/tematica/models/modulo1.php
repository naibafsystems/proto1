...
class Modulo1 extends CI_Model {
    ...
    function descargaPlanosModulo($ano_periodo, $mes_periodo){
    	$modulo1 = array();
    	$sql = "SELECT CTRL.ano_periodo, CTRL.mes_periodo, CTRL.nro_orden, CTRL.nro_establecimiento, EMP.idnit, EMP.idproraz, EMP.idnomcom, EMP.idsigla, EMP.iddirecc, EMP.fk_depto, EMP.fk_mpio, EMP.idtelno, EMP.idfaxno, EMP.idpagweb, EMP.idcorreo, EST.finicial, EST.ffinal,
       				   ...
       				   CTRL.fk_novedad, CTRL.fk_estado
				FROM rmmh_admin_control CTRL, rmmh_admin_empresas EMP, rmmh_admin_establecimientos EST
				WHERE CTRL.nro_orden = EMP.nro_orden
				AND CTRL.nro_establecimiento = EST.nro_establecimiento
				AND EMP.nro_orden = EST.nro_orden
				AND CTRL.ano_periodo = $ano_periodo
    	...
    	if ($query->num_rows()>0){
    		...
    		foreach ($query->result() as $row){
    			...
    			$modulo1[$i]["idproraz"] = $row->idproraz;
      			...
      			$modulo1[$i]["estado"] = $this->novedad->nombreEstadoFormulario($row->fk_novedad, $row->fk_estado);
      			$i++;
    		}
    	}
    	$this->db->close();
