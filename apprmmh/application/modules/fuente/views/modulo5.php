...
<br/>
<p>De acuerdo a la  modalidad principal de alojamiento que usted presta, seleccione una de las seis categor&iacute;as presentadas (hoteles, aparta-hoteles, centros vacacionales, alojamiento rural, hostales, zonas de camping). En esta se&ntilde;ale los servicios caracter&iacute;sticos de su establecimiento.</p>
<!-- br/>
	     	...
	     	$valor = $i+1;
       		...
       		if ($contador<3){
	     		...
	     		if (strrpos($stringDatos,"1")){ //Si viene un uno en alguna de las cadenas de string-checks, debo marcar toda la categoria
	     			...
	     			echo '<b>'.$categorias[$i]["cod_ciiu"].' - '.$categorias[$i]["nombre"].'</b>&nbsp;<input type="checkbox" id="chkCategoria'.$valor.'" name="chkCategoria'.$valor.'" value="'.$valor.'" disabled/>';
	     		}
	     		echo '<br/>';	     		
	     		...
	     		for ($j=0; $j<count($servicios); $j++){
	     			...
	     			if (isset($stringDatos[$j])&&($stringDatos[$j]==1)){
	     				...
	     				echo '<td valign="top"><input type="checkbox" id="chkServicios'.$valor.$serv.'" name="chkServicios'.$valor.$serv.'" value="x" disabled/></td>';
	     			}		
...
<br/>
<?php if (!$bloqueo){ ?>
	<center><input type="button" id="btnModuloV" name="btnModuloV" value="Guardar y continuar" class="button"/></center>
<?php } ?>
