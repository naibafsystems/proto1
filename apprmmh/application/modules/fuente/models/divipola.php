...
class Divipola extends CI_Model {
	...
	function nombreDepartamento($id){
		...
		if ($query->num_rows()>0){
			foreach($query->result() as $row){
				$nombre = strtoupper(utf8_encode($row->nom_depto));
			}
		}
		$this->db->close();
		return $nombre;
    ...
    function obtenerDepartamentos(){
    	...
    	if ($query->num_rows()>0){
    		...
    		foreach ($query->result() as $row){
      			...
      			$departamentos[$i]["nombre"] = utf8_encode($row->nom_depto);
      			$i++;
   			}
    	}
    	$this->db->close();
    ...
    function nombreMunicipio($id){
		...
		if ($query->num_rows()>0){
			foreach($query->result() as $row){
				$nombre = strtoupper(utf8_encode($row->nom_mpio));
			}
		}
		$this->db->close();
		return $nombre;
    ...
    function obtenerMunicipios($depto){
    	...
    	if ($query->num_rows()>0){
    		...
    		foreach ($query->result() as $row){
    			...
    			$municipios[$i]["nombre"] = utf8_encode($row->nom_mpio);
    			$i++;
    		}
    	}
    	$this->db->close();
