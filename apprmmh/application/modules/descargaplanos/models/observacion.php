...
class Observacion extends CI_Model {
    ...
    function obtenerObservaciones($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    	...
    	if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
    			...
    			$observaciones[$i]["observacion"] = utf8_decode($row->descripcion);    			
    			$i++;
    		}   			
   		}
		$this->db->close();
	...
	function obtenerObservacionesModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, $modulo){
    	...
    	if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
    			...
    			$observaciones[$i]["modulo"] = $this->obtenerNombreCapitulo($row->modulo);
    			$observaciones[$i]["campo"] = $row->nom_campo;    			
    			$observaciones[$i]["descripcion"] = utf8_decode($row->descripcion);
    			$i++;
    		}   			
   		}
		$this->db->close();
	...
	function guardarObservacion($modulo, $campo, $observacion, $fecha, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, $usuario){
    	$data = array ('modulo' => $modulo, 
    	               'nom_campo' => $campo,
    	               'descripcion' => utf8_encode($observacion),
    	               'fecha' => $fecha,
    	               'nro_orden' => $nro_orden,
    ...
    function descargaPlanosObservaciones($ano_periodo, $mes_periodo){
    	...
    	$sql = "SELECT C.nro_orden, C.nro_establecimiento, C.ano_periodo, C.mes_periodo, OB.fecha, OB.modulo,
                       CASE modulo
                          WHEN 1 THEN 'Modulo I'
                          WHEN 2 THEN 'Modulo II'
                          WHEN 3 THEN 'Modulo III'
                          WHEN 4 THEN 'Modulo IV'
                          WHEN 0 THEN 'Envio del formulario'
                          WHEN 99 THEN 'Ficha de analisis'
                       END nom_modulo, descripcion AS observacion, IFNULL(HN.fk_novedad,C.fk_novedad) AS fk_novedad, C.fk_estado
                ...
                LEFT JOIN rmmh_admin_observaciones OB ON (C.nro_orden = OB.nro_orden
                                                          AND C.nro_establecimiento = OB.nro_establecimiento
                                                          AND C.ano_periodo = OB.ano_periodo
                                                          AND C.mes_periodo = OB.mes_periodo)
    	...
    	if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
    			...
    			$observaciones[$i]["descripcion"] = utf8_decode($row->observacion);
    ...
    function obtenerNombreCritico($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    	...
    	if ($query->num_rows() > 0){
    		$i = 0;
			foreach ($query->result() as $row){
      			$nombre = utf8_decode($row->nom_usuario);
    		}   			
   		}
		$this->db->close();
